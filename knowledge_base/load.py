...
def load_from_local_path(root_path, allowed_extensions=config["allowed_extensions"]):
    ...
    documents = []
    logging.info(f"Loading files from local path: {root_path}...")
    ...
    logging.info(f"Found {len(documents)} files in {root_path}.")
    return documents
...
def load_from_github(repo_url, allowed_extensions=config["allowed_extensions"]):
    ...
    logging.info(f"Cloning github repo: {repo_url}...")
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            subprocess.run(['git', 'clone', '--depth', '1', repo_url, temp_dir], check=True, capture_output=True)
            return load_from_local_path(temp_dir, allowed_extensions)
        except subprocess.CalledProcessError as e:
            logging.error(f"Failed to clone repository {repo_url}. Error: {e.stderr.decode()}")
            return []
